...
double gd(double a, double b, double x)
{
    double c = (x-b) / a;
    return exp((-.5) * c * c) / (a * sqrt(2 * PI));
}

FVec make_gv(double a, double x0, double x1, unsigned int length)
{
    FVec v;
    v.length = length;
    v.data = malloc(length * sizeof(double));

    double step = (x1 - x0) / ((double)length);
    int offset = length/2;
    ...
    for (y = 0; y < a.dimY; y++)
    {
        for (x = 0; x < a.dimX; x++)
        {
            ...
            for (channel = 0; channel < a.numChannels; channel++)
            {
                ...
                {
                    offset = i - ext;
                    sum += gv.data[i] * (double)get_pixel(a, x + offset, y)[channel];
                }
    ...
    for (y = 0; y < a.dimY; y++)
    {
        for (x = 0; x < a.dimX; x++)
        {
            ...
            for (channel = 0; channel < a.numChannels; channel++)
            {
                ...
                {
                    offset = i - ext;
                    sum += gv.data[i] * (double)get_pixel(a, x, y + offset)[channel];
                }
                pc[channel] = (unsigned char)sum;
            }
        }
...
int main(int argc, char** argv)
{
    if (argc < 6)
    {
        printf("Usage: ./gb.exe <inputjpg> <outputname> <double: a> <double: x0> <double: x1> <unsigned int: dim>\n");
        exit(0);
    }
